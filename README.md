...
2. Encontrar, de forma eficiente, o **menor caminho** entre o restaurante e cada ponto de entrega, considerando as restrições urbanas (ruas, distâncias e congestionamento);
3. Em cenários com muitos pedidos simultâneos, **agrupar entregas próximas em zonas**, para que cada entregador atenda uma região coesa em vez de se deslocar de forma dispersa pela cidade.

**Objetivos do projeto:**

- Modelar a malha viária da região central como um grafo ponderado;
...
Pré-requisitos: Python 3.10+ e as bibliotecas `networkx`, `scikit-learn`, `numpy` e `matplotlib`.

```bash
pip install networkx scikit-learn numpy matplotlib

# (opcional) regenerar os dados de ruas e pedidos a partir dos bairros
python src/gerar_dados.py

# rodar o pipeline completo: A*, BFS, DFS, K-Means e geração dos gráficos
python src/main.py
```

Os gráficos serão salvos em `docs/` e um resumo numérico dos resultados em `docs/resultados.txt`.
